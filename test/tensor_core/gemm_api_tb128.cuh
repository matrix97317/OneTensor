                              ...
                              int *a_offset)
{
    ...
    int row = lane_id % 16;
    int col = lane_id / 16;
    int a_row_offset = ((block_row * warp_row_num * mma_row_num + warp_row * mma_row_num + mma_row) * mma_m);
    ...
    int *b_offset)
{
    ...
    int row = lane_id % 16;
    int col = lane_id / 16;
    int b_col_offset = ((block_col * warp_col_num * mma_col_num + warp_col * mma_col_num + mma_col) * mma_n);
...
{
    // A share mem offset
    *a_smem_offset = (warp_row * mma_row_num + mma_row) * (16 * 16) + lane_id * 8;
}
...
{
    // B share mem offset
    *b_smem_offset = (warp_col * mma_col_num + mma_col) * (16 * 16) + lane_id * 8;
}


__global__ void gemm_fp16fp32_tb128(half *a, half *b, float *c,
                                    int M, int N, int K)
{
    ...
    __shared__ half a_smem[2 * STAGE][16 * 16 * WARP_ROW_NUM * MMA_ROW_NUM];
    __shared__ half b_smem[2 * STAGE][16 * 16 * WARP_COL_NUM * MMA_COL_NUM];
    float c_register0[MMA_ROW_NUM][MMA_COL_NUM][4];
    float c_register1[MMA_ROW_NUM][MMA_COL_NUM][4];

    uint32_t a_register[2][MMA_ROW_NUM][4];
    uint32_t b_register[2][MMA_COL_NUM][4];
...
#pragma unroll
    for (; stage_id < (STAGE - 1); stage_id++)
    {
        // A MMA_ROW_NUM
#pragma unroll
        for (int mma_row_id = 0; mma_row_id < MMA_ROW_NUM; mma_row_id++)
        {
            int a_k_tiles = stage_id*2 + warp_col;
            smem_a_offset(warp_row,
                         MMA_ROW_NUM,
                         mma_row_id,
                         lane_id,
                         &a_smem_offset);
        ...
        // B MMA_COL_NUM
#pragma unroll
        for (int mma_col_id = 0; mma_col_id < MMA_COL_NUM; mma_col_id++)
        {
            int b_k_tiles = stage_id*2 + warp_row;
            smem_b_offset(warp_col,
                         MMA_COL_NUM,
                         mma_col_id,
                         lane_id,
                         &b_smem_offset);
...
#pragma unroll
    for (int k = 2 * (STAGE - 1); k < k_tiles; k = k + 2)
    {
...
#pragma unroll
        for (int mma_row_id = 0; mma_row_id < MMA_ROW_NUM; mma_row_id++)
        {
            ...
            a_gmem_offset(block_row,
                          ...
                          MMA_ROW_NUM,
                          mma_row_id,
                          k + warp_col, K, MMA_M, MMA_K, lane_id,
                          &a_offset);
            CP_ASYNC_CG((uint32_t)__cvta_generic_to_shared(&a_smem[save_stage_id * 2 + warp_col][a_smem_offset]), &a[a_offset], 16);
        }
...
#pragma unroll
        for (int mma_col_id = 0; mma_col_id < MMA_COL_NUM; mma_col_id++)
        {
            ...
            b_gmem_offset(block_col,
                          ...
                          MMA_COL_NUM,
                          mma_col_id,
                          k + warp_row, N, MMA_N, MMA_K, lane_id,
                          &b_offset);
            CP_ASYNC_CG((uint32_t)__cvta_generic_to_shared(&b_smem[save_stage_id * 2+warp_row][b_smem_offset]), &b[b_offset], 16);
        }
...
#pragma unroll
        for(int i=0; i<MMA_ROW_NUM;i++){
            mma_row = i;
            load_a_smem_to_reg(&a_smem[0 + stage_id * 2][(warp_row * MMA_ROW_NUM + mma_row) * (16 * 16)], a_register[0][mma_row], lane_id);
...
#pragma unroll
        for(int j=0; j<MMA_COL_NUM;j++){
            mma_col=j;
            load_b_smem_to_reg(&b_smem[0 + stage_id * 2][(warp_col * MMA_COL_NUM + mma_col) * (16 * 16)], b_register[0][mma_col],lane_id);
...
#pragma unroll
        for(int i=0; i<MMA_ROW_NUM;i++){
            mma_row = i;
            load_a_smem_to_reg(&a_smem[0 + tail_stage_id * 2][(warp_row * MMA_ROW_NUM + mma_row) * (16 * 16)], a_register[0][mma_row], lane_id);
...
#pragma unroll
        for(int j=0; j<MMA_COL_NUM;j++){
            mma_col=j;
            load_b_smem_to_reg(&b_smem[0 + tail_stage_id * 2][(warp_col * MMA_COL_NUM + mma_col) * (16 * 16)], b_register[0][mma_col],lane_id);
