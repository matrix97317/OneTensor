...
__global__ void gemm_fp16fp32_tb256(half *a, half *b, float *bc,
                                    int M, int N, int K)
{
    ...
    int stride_k = div_ceil(k_tiles,SPLIT_K);
    int k_start = blockIdx.x* stride_k;
    int k_end = (blockIdx.x+1) * stride_k;
    if (blockIdx.x==(SPLIT_K-1)){
        k_end = k_tiles;
    }
    float *c = &bc[blockIdx.x* M*N];
    // __syncthreads();
...
#pragma unroll
        for (int mma_row_id = 0; mma_row_id < (MMA_ROW_NUM/MMA_ROW_SPLIT); mma_row_id++)
        {
            ...
            a_gmem_offset(block_row,
                          ...
                          MMA_ROW_NUM,
                          mma_row_id,
                          a_k_tiles+k_start, K, MMA_M, MMA_K, lane_id,
                          &a_offset);
...
#pragma unroll
        for (int mma_col_id = 0; mma_col_id < (MMA_COL_NUM/MMA_COL_SPLIT); mma_col_id++)
        {
            ...
            b_gmem_offset(block_col,
                          ...
                          MMA_COL_NUM,
                          mma_col_id,
                          b_k_tiles+k_start, N, MMA_N, MMA_K, lane_id,
                          &b_offset);
...
#pragma unroll
    for (int k = 1 * (STAGE - 1)+k_start; k < k_end; k = k + 1)
    {
...
#pragma unroll
        ...
        CP_ASYNC_COMMIT_GROUP();
        // CP_ASYNC_WAIT_GROUP((((STAGE-2)>0)?(STAGE-2):1)); //stage 3-4
        
        CP_ASYNC_WAIT_GROUP(1); //stage 2
...
//                           &c_row1_offset_0,
//                           &c_row1_offset_1);
//             for(int spk=1; spk<SPLIT_K; spk++){
//                 float *temp_c = &bc[spk*M*N];
//                 c[c_row0_offset_0]=c[c_row0_offset_0]+temp_c[c_row0_offset_0];
//                 c[c_row0_offset_0+1]=c[c_row0_offset_0+1]+temp_c[c_row0_offset_0+1];

//                 c[c_row1_offset_0]=c[c_row1_offset_0]+temp_c[c_row1_offset_0];
//                 c[c_row1_offset_0+1]=c[c_row1_offset_0+1]+temp_c[c_row1_offset_0+1];

//                 c[c_row0_offset_1]=c[c_row0_offset_1]+temp_c[c_row0_offset_1];
//                 c[c_row0_offset_1+1]=c[c_row0_offset_1+1]+temp_c[c_row0_offset_1+1];

//                 c[c_row1_offset_1]=c[c_row1_offset_1]+temp_c[c_row1_offset_1];
//                 c[c_row1_offset_1+1]=c[c_row1_offset_1+1]+temp_c[c_row1_offset_1+1];
//             }
...
__global__ void reduce_matrix(float*c, float* out, int M, int N, int K){
    int tid = blockIdx.x*blockDim.x+threadIdx.x;
    if(tid<(M*N)){
#pragma unroll
        for(int k=0;k<K;k++){
            float *temp_c = &c[k*M*N];
            out[tid]=out[tid]+temp_c[tid];
        }
    }
}

void gemm_split_k(half *a, half *b, float *bc,float* out,int M, int N, int K,int split_k,cudaStream_t stream){
      dim3 gird(SPLIT_K, div_ceil(M, (WARP_ROW_NUM * MMA_ROW_NUM * MMA_M)), div_ceil(N, (WARP_COL_NUM * MMA_COL_NUM * MMA_N * 1)));
      dim3 block(32 * 8, 1, 1);
      gemm_fp16fp32_tb256<<<gird, block, 0, stream>>>(a, b, bc, M, N, K);
      dim3 gird1(div_ceil(M*N, 128));
      dim3 block1(128, 1, 1);
      reduce_matrix<<<gird1, block1, 0, stream>>>(bc,out, M, N, split_k);
}
#endif