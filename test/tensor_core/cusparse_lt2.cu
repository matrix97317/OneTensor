...
int main(void) {
    ...
    auto     A_size         = A_height * lda * sizeof(DT);
    auto     B_size         = B_height * ldb * sizeof(DT);
    auto     C_size         = C_height * ldc * sizeof(DT);
    ...
    for (int64_t i = 0; i < m * k; i++)
        hA[i] = static_cast<DT>(static_cast<float>(std::rand() % 10));
    for (int64_t i = 0; i < k * n; i++)
        hB[i] = static_cast<DT>(static_cast<float>(std::rand() % 10));
    float alpha = 1.0f;
    float beta  = 0.0f;
