...
void print_matrix(const DT* matrix,
                  int64_t       height,
                  int64_t       width,
                  int64_t       ld,
                  int           num_batches,
...
DT random_half_gen() {
    return static_cast<DT>(static_cast<float>(std::rand() % 10));
}
...
int main(void) {
    ...
    constexpr bool print_sparse_matrix = false;
    // Host problem definition, row-major order
    constexpr int     num_batches   = 1;
    constexpr int64_t m             = 32*60;
    constexpr int64_t n             = 256;
    constexpr int64_t k             = 1024;
    constexpr int64_t batch_strideA = m * k *1;
    constexpr int64_t batch_strideB = k * n *1;
    constexpr int64_t batch_strideC = m * n *1;
    ...
    constexpr auto    type          = CUDA_R_8I;
    constexpr auto    compute_type  = CUSPARSE_COMPUTE_32I;
    ...
    auto     A_size_bytes   = num_batches * batch_strideA * sizeof(DT);
    auto     B_size_bytes   = num_batches * batch_strideB * sizeof(DT);
    auto     C_size_bytes   = num_batches * batch_strideC * sizeof(DT);
    auto     hA             = new DT[A_size];
    auto     hB             = new DT[B_size];
    auto     hC             = new DT[C_size]();
    for (int b = 0; b < num_batches; b++) {
    ...
    int splitKBuffers =4;
    // DT* splitKBuffers;
    // splitKBuffers = new DT[m*n*splitK];
    // for (int i = 0; i < m; i++)
    //     hBias[i] = 1.0f;
    ...
    timer.stop();
    float ms = timer.elapsed_millis()/100;
